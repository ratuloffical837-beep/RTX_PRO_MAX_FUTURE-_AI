      ...
      <div style={{ padding: '12px 16px' }}>
        <Card center>
          ...
          <div style={{ fontSize: 11, color: C.muted, marginTop: 10 }}>
            Sweep Reclaim • 200 Indicators • Hybrid Mode
          </div>
          ...
          {tgUser?.id && (
            <div style={{
              ...
              {tgUser.username && <span style={{ color: C.cyan }}> @{tgUser.username}</span>}
            </div>
          )}
...
function TopBanner() {
  return (
    <div style={{
      ...
      🚀 <strong>RTX Pro Max</strong> ৳{MONTHLY_AMOUNT}/মাস | ২৪ ঘন্টা ফ্রি ট্রায়াল 🎁
    </div>
  )
}
...
function SocialButtons() {
  return (
    <div style={{ display: 'flex', gap: 8, marginBottom: 12, marginTop: 12 }}>
      <button onClick={() => window.open(GROUP_LINK, '_blank')} style={socialBtn(C.cyan)}>
        💬 Group
      </button>
...
function SupportButton() {
  return (
    <button onClick={() => window.open(SUPPORT_LINK, '_blank')} style={{
      ...
      background: `${C.cyan}11`, color: C.cyan,
      fontWeight: 700, fontSize: 13,
      border: `1px solid ${C.cyan}44`, cursor: 'pointer', marginTop: 12,
    }}>
      💬 Customer Support
    </button>
  )
...
function NumCard({ onClick, color, emoji, label, num, copied }) {
  return (
    <div onClick={onClick} style={{
      ...
      border: `1px solid ${color}55`, transition: '0.2s',
    }}>
      ...
      <div style={{
        fontSize: 10, color: copied ? C.green : C.muted, marginTop: 4,
      }}>
...
const socialBtn = (color) => ({
  ...
  background: `${color}11`, color, fontWeight: 700, fontSize: 12,
