...
export default function SpotSection({ isTrial, tgUser, refreshCount, setRefreshCount, signalMode }) {
  ...
  const generateSignal = useCallback(async () => {
    ...
    const modeName = signalMode === 'sweep' ? 'Sweep Reclaim' :
                     signalMode === 'indicator' ? '200 Indicators' :
                     'Hybrid Analysis'

    let prog = 0
    const messages = [
      `🔍 Initializing ${modeName}...`,
      '📊 Fetching 4H timeframe data...',
      '📊 Fetching 1H timeframe data...',
      '📊 Fetching 30m timeframe data...',
      '📊 Fetching 15m + 5m + 1m data...',
    ...
    progressTimer.current = setInterval(() => {
      prog = Math.min(prog + Math.random() * 5, 95)
      setProgress(prog)
      if (msgIdx < messages.length && prog > (msgIdx + 1) * 7) {
    ...
    <div style={{ padding: '12px', display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {generating && (
        <div style={{
          ...
          <div style={{
            height: 6, background: C.panel, borderRadius: 3,
            overflow: 'hidden',
          }}>
            <div style={{
              height: '100%',
          ...
          <div style={{
            ...
            {Math.round(progress)}% • {signalMode === 'sweep' ? 'Sweep Reclaim' :
                                      signalMode === 'indicator' ? '200 Indicators' :
                                      'Hybrid'} Mode
          </div>
        </div>
        ...
        <div style={{
          ...
          {blocked && (
            <div style={{ fontSize: 11, color: C.muted, marginTop: 8 }}>
              💎 Premium নিন Unlimited Signals এর জন্য
            </div>
          )}
        </div>
      )}
