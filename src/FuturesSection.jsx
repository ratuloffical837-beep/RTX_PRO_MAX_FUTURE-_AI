...
export default function FuturesSection({ isTrial, tgUser, refreshCount, setRefreshCount, signalMode }) {
  const [selectedCoin, setSelectedCoin] = useState(localStorage.getItem('rtx_futures_coin') || 'BTCUSDT')
  const [favorites, setFavorites] = useState(JSON.parse(localStorage.getItem('rtx_futures_favs') || '["BTCUSDT","ETHUSDT"]'))
  ...
  return (
    <div style={{ padding: '12px', display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {/* Progress */}
      {generating && (
        <div style={{ background: C.card, borderRadius: 12, padding: 16, border: `1px solid ${C.orange}55` }}>
          <div style={{ fontSize: 11, color: C.orange, fontWeight: 700, marginBottom: 10, textAlign: 'center' }}>
      ...
      <div style={{ background: C.card, borderRadius: 12, padding: 14, border: `1px solid ${C.border}` }}>
        ...
        <div style={{ fontSize: 10, color: C.muted, fontWeight: 700, marginBottom: 8 }}>⚡ TOP 20 BINANCE FUTURES</div>
...
function CoinChip({ coin, selected, onClick, onFav, isFav, price, funding }) {
  ...
  return (
    <div style={{
      background: selected ? `linear-gradient(135deg, ${C.orange}33, ${C.pink}33)` : C.panel,
      ...
      <div onClick={onClick} style={{ flex: 1 }}>
        <div style={{ fontSize: 12, fontWeight: 800, color: selected ? C.orange : C.text }}>{symbol}</div>
        {price && (
        ...
        {funding !== undefined && (
          <div style={{ fontSize: 8, fontWeight: 600, color: funding > 0 ? C.green : C.red, marginTop: 1 }}>
