...
export default function SignalCard({ signal, coin, market, isTrial, livePrice }) {
  ...
  return (
    <div style={{
      ...
      {/* SMC Analysis */}
      {signal.smcAnalysis && Object.values(signal.smcAnalysis).some(v => v !== 'N') && (
        <div style={{
      ...
      {signal.indicators && signal.indicators.length > 0 && (
        <details style={{ marginTop: 12 }}>
          <summary style={{
            ...
            📊 View Detailed Analysis ({signal.indicators.length} items)
          </summary>
          <div style={{
            ...
            {signal.indicators.map((ind, i) => (
              <div key={i} style={{
                ...
                <span style={{
                  ...
                  {typeof ind.signal === 'string' && ind.signal.includes('BULL') ? '↑' :
                   typeof ind.signal === 'string' && ind.signal.includes('BEAR') ? '↓' :
                   ind.signal || '→'}
                </span>
              </div>
            ))}
...
function TPSLDisplay({ tp, isLong, market }) {
  if (!tp) return null
