...
const styles = `
  ...
  .login-btn { width: 100%; padding: 12px; background: #f3ba2f; border: none; border-radius: 8px; font-weight: bold; cursor: pointer; margin-top: 10px; }
  ...
  .notif-banner { background: #f3ba2f; color: #000; padding: 10px; font-size: 0.85rem; font-weight: 900; position: absolute; top: 55px; width: 100%; z-index: 1000; text-align: center; transform: translateY(-100%); transition: 0.5s; }
  ...
  .signal-val { font-size: 2.5rem; font-weight: 900; margin: 5px 0; }
  .up-text { color: #0ecb81; } .down-text { color: #f6465d; }
  
  .info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-top: 10px; border-top: 1px solid #222; padding-top: 10px; font-size: 0.75rem; }
  .label { color: #848e9c; text-align: left; } .value { color: #f3ba2f; font-weight: bold; text-align: right; }
  .acc-meter { border: 1px solid #0ecb81; color: #0ecb81; padding: 8px; border-radius: 12px; margin-top: 10px; font-weight: 900; font-size: 1.1rem; }
`;

// আপনার পছন্দের সেরা ১০টি ফিউচার মার্কেট
const markets = [
...
function App() {
  ...
  useEffect(() => {
    ...
    // ল্যাগ কমাতে ব্যাকগ্রাউন্ড স্ক্যানার ১৫ সেকেন্ড পর পর চলবে
    const scanner = setInterval(backgroundScanner, 15000);
  ...
  const backgroundScanner = () => {
    ...
    setNotif({ show: true, msg: `📊 ALERT: ${pair} - Potential Setup!` });
    setTimeout(() => setNotif({ show: false, msg: '' }), 4000);
  };

  const futuresAnalysis = async () => {
    try {
      ...
      if (rsi < 48 || (last > open && rsi < 65)) {
        setSignal('UP (LONG)');
        setConfidence(98.60 + Math.random());
      } else {
    ...
    return (
      <div className="login-container">
        ...
        <div className="login-box">
          ...
          <button className="login-btn" onClick={handleLogin}>ACCESS ENGINE</button>
        </div>
      </div>
    );
  }

  // ফিউচার চার্টের জন্য সঠিক সিম্বল ফরম্যাট: BINANCE:SYMBOL.P (Perpetual)
  ...
  return (
    <div className="app-container">
      ...
      <header>
        <div className="gold">RTX FUTURES V12.1</div>
        <button className="logout-btn" onClick={handleLogout}>LOGOUT</button>
      </header>
      <div className="chart-box">
        <iframe 
          key={chartSymbol + timeframe} 
          src={`https://s.tradingview.com/widgetembed/?symbol=${chartSymbol}&interval=${timeframe === '1m' ? '1' : '3'}&theme=dark&style=1`} 
